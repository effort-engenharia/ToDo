...
const ApontamentosComercial = ({ onVoltar, onDataUpdate }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    cidadeAtendimento: '',
    cidadeOutras: '',
    cronogramaDataInicio: '',
    cronogramaDataTermino: ''
  });
  ...
  const consultarCnpj = async (cnpj) => {
    ...
    try {
      ...
      showToast('❌ Erro ao consultar CNPJ. Verifique o número informado.', 'error');
  ...
  const validateForm = () => {
    ...
    if (formData.cronogramaDataInicio && formData.cronogramaDataTermino) {
      const dataInicio = new Date(formData.cronogramaDataInicio);
      const dataTermino = new Date(formData.cronogramaDataTermino);
      
      if (dataTermino <= dataInicio) {
        newErrors.cronogramaDataTermino = 'Data de término deve ser posterior à data de início';
      }
    }
      ...
      {isFormVisible && (
        <div className="max-w-4xl mx-auto p-4 sm:p-6">
          <div className="bg-white rounded-xl sm:rounded-2xl p-6 sm:p-8 shadow-xl">
            ...
            {/* Grid para Cronograma */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Data Início do Cronograma */}
              <div>
                <label className="flex items-center text-sm font-medium text-gray-700 mb-2">
                  <FaCalendarAlt className="mr-2 text-green-500" />
                  Cronograma: Data Início
                </label>
                <input
                  type="date"
                  value={formData.cronogramaDataInicio}
                  onChange={(e) => setFormData(prev => ({ ...prev, cronogramaDataInicio: e.target.value }))}
                  className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                    errors.cronogramaDataInicio ? 'border-red-500' : 'border-gray-300'
                  }`}
                />
                {errors.cronogramaDataInicio && (
                  <p className="text-red-500 text-sm mt-1">{errors.cronogramaDataInicio}</p>
                )}
              </div>

              {/* Data Término do Cronograma */}
              <div>
                <label className="flex items-center text-sm font-medium text-gray-700 mb-2">
                  <FaCalendarAlt className="mr-2 text-red-500" />
                  Cronograma: Data Término
                </label>
                <input
                  type="date"
                  value={formData.cronogramaDataTermino}
                  onChange={(e) => setFormData(prev => ({ ...prev, cronogramaDataTermino: e.target.value }))}
                  className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                    errors.cronogramaDataTermino ? 'border-red-500' : 'border-gray-300'
                  }`}
                />
                {errors.cronogramaDataTermino && (
                  <p className="text-red-500 text-sm mt-1">{errors.cronogramaDataTermino}</p>
                )}
              </div>
            </div>
